-- Bidirectional workspace sync: retain client freshness metadata and expose
-- authenticated RPCs so a stale browser cannot overwrite a newer remote row.
alter table public.workspaces add column if not exists key text;
alter table public.board_columns add column if not exists created_at timestamptz not null default now();
alter table public.board_columns add column if not exists revision integer not null default 1;
alter table public.board_cards add column if not exists ticket_number integer;
...
begin
  -- Sync RPCs supply the client edit time. Ordinary updates still receive now().
...
revoke all on function public.hckr_sync_snapshot() from public, anon;
revoke all on function public.hckr_apply_sync_batch(jsonb) from public, anon;
grant execute on function public.hckr_sync_snapshot() to authenticated;
grant execute on function public.hckr_apply_sync_batch(jsonb) to authenticated;
